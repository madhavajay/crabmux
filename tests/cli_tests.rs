...
fn test_new_command_variations() {
    ...
    let output = cmd.arg("new")
        .arg(&unique_session_name)
        .output()
        .unwrap();
    ...
    if output.status.success() {
        let _ = std::process::Command::new("tmux")
            .args(&["kill-session", "-t", &format!("{}-alias", unique_session_name)])
            .output();
    }
...
fn test_restore_command_without_file() {
    ...
    cmd.arg("restore")
        .assert()
        .failure(); // Will likely fail because default snapshot file doesn't exist
}
...
fn test_alias_command_variations() {
    ...
    cmd.arg("alias")
        .arg("test-alias")
        .assert()
        .success(); // Should succeed even if alias doesn't exist
...
fn test_kill_all_command() {
    ...
    if output.status.success() {
        ...
        assert!(
            ...
            stdout.contains("No tmux sessions to kill")
        );
    }
}
